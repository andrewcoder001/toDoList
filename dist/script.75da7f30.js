...
function addGlobalEventListener(listener, property, callback) {
...
function setupToDoList(transferTask) {
  (0, _addGlobalEventListener.default)('mousedown', '[data-task-element]', function (e) {
    var selectedTask = e.target.closest('[data-task]'); //on click, will get selected element based on data attribute, [data-task] 

    var taskClone = selectedTask.cloneNode(true); //cloneNode(true) clones 'selectedTask' and its child elements

    var shadow = selectedTask.cloneNode(); // Runs setupDragItems and stores return value in variable to be used as an argument for 'setupDragEvents'
...
function setupDragItems(selectedTask, taskClone, shadow, e) {
  var taskRect = selectedTask.getBoundingClientRect(); //stopped on offset
  ...
  taskClone.classList.add('dragging'); // Positioning of Clone     

  positionClone(taskClone, e, offset); // Append to Document

  document.body.append(taskClone); // Shadow Setup

  shadow.style.height = "".concat(taskRect.height, "px");
...
function positionClone(taskClone, mousePosition, offset) {
  taskClone.style.top = "".concat(mousePosition.clientY - offset.y, "px");
  taskClone.style.left = "".concat(mousePosition.clientX - offset.x, "px");
}
},{"../utils/addGlobalEventListener":"../utils/addGlobalEventListener.js"}],"../node_modules/uuid/dist/esm-browser/rng.js":[function(require,module,exports) {
"use strict";
...
var STORAGE_PREFIX = 'TODO_LIST';
var STORAGE_KEY = "".concat(STORAGE_PREFIX, "_column");
...
var columns = loadColumns(); // Event Listeners

form.addEventListener('submit', function (e) {
  e.preventDefault();
  var taskHeader = document.querySelector('.task-subject').value;
  var taskContent = document.querySelector('.task-content').value;
  var inputFieldHeader = document.querySelector('.task-subject');
  var inputFieldContent = document.querySelector('.task-content');
...
function clearColumns() {
  var bothColumns = document.querySelectorAll('[data-task-column]');
  bothColumns.forEach(function (col) {
    return col.innerHTML = '';
  }); // loop through object keys and turn pair into array

  Object.entries(columns).forEach(function (obj) {
    // each loop, will grab each array element's first element.
    var columnName = obj[0];
    columns[columnName] = [];
  });
  saveColumns();
...
function loadColumns() {
  ...
  var columnsJSON = localStorage.getItem(STORAGE_KEY); //parse the JSON item if there are any (Truthy), if not, use default values.
...
function renderTasks() {
  ...
  Object.entries(columns).forEach(function (obj) {
    ...
    var columnName = obj[0];
    var tasks = obj[1];
    var column = document.querySelector("[data-column-name=\"".concat(columnName, "\"]")); // clear column first
...
function createTaskElement(task) {
  // Task template div
  var element = document.createElement('div'); // Data attributes

  element.dataset.id = task.id;
  element.dataset.task = ''; // Template child elements

  element.classList.add('task');
  element.innerHTML = "\n            <div class=\"task-header\" data-task-element>".concat(task.taskHeader, "</div>\n            <p data-task-element>").concat(task.taskContent, "</p>\n        ");
  return element;
} // Prop Transfer
...
function transferTask(e) {
  ...
  var startColumnName = e.startColumn.dataset.columnName;
  var endColumnName = e.endColumn.dataset.columnName; // Has to be bracket to use variable name; dot notation is used to access straight to the key.

  var startColumnTasks = columns[startColumnName];
  var endColumnTasks = columns[endColumnName];
  var task = startColumnTasks.find(function (t) {
    return t.id == e.draggingTask.dataset.id;
  }); //Find index of 'task' inside startColumnTasks, remove it

  startColumnTasks.splice(startColumnTasks.indexOf(task), 1); //Put in task at the 'shadow's index position
...
renderTasks();
},{"./dragAndDrop":"dragAndDrop.js","uuid":"../node_modules/uuid/dist/esm-browser/index.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
